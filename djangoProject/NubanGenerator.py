...
class NJbank:
    def __init__(self):
        ...
        self.ofiUniqueCode = "940091"
        self.nuban = 0
        self._checksum = ""

    def generate(self, ofiUniqueCode):
        self.nuban = self.nubanNumberGenerate()
        self.ofiUniqueCode = ofiUniqueCode
        # CHECK FOR ERRORS
        if(len(self.ofiUniqueCode) != 6):
            raise Exception("OFI unique code must be of length 6 and must be a string")
    ...
    def checkSum(self):
        ...
        # print(len(self.accountNumber))
        for x in range(len(self.accountNumber)):
            # print(x)
            # print(int(self.accountNumber[x])*self.digits[x])
